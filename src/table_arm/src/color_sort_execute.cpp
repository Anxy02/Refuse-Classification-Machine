...
void arm_put(std::string color)
{
   ...
   //根据色块的颜色判断放置位置
         if (color == "yellow") {arm.setNamedTarget("right_pose");  arm.move();  sleep(1); }
    else if (color == "blue")   {arm.setNamedTarget("right_pose");    arm.move();  sleep(1); }
    else if (color == "green")  {arm.setNamedTarget("right_pose");   arm.move();  sleep(1);ROS_INFO("sorting color is done");}
